...
// Position of the eagle's (viewer-)left eye inside the head image.
const EYE_X = 32; // %
const EYE_Y = 31.5; // %
...
export function EagleGate() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (v) => {
    eyeParams.current = {
      pupil: pupilAt(v),
      rage: Math.min(1, Math.max(0, (v - 0.5) / 0.35)),
      rotation: v * 0.9,
    };
  });

  /* ---- head layer: idle breathe, then dive into its left eye ---- */
  const headScale = useTransform(scrollYProgress, [0, 0.34, 0.5], [1, 1.07, 5.6]);
  const headX = useTransform(scrollYProgress, [0, 0.34, 0.5], ["0%", "0%", `${50 - EYE_X}%`]);
  const headY = useTransform(scrollYProgress, [0, 0.34, 0.5], ["0%", "0%", `${50 - EYE_Y}%`]);
  const headOpacity = useTransform(scrollYProgress, [0.46, 0.54], [1, 0]);

  /* ---- procedural eye: takes over inside the photo eye ---- */
  const eyeOpacity = useTransform(scrollYProgress, [0.44, 0.52], [0, 1]);
  const eyeScale = useTransform(
    scrollYProgress,
    [0.44, 0.52, 0.68, 0.95, 1],
    [0.9, 1, 1.16, 3.4, 9]
  );
  ...
  const focus = useTransform(scrollYProgress, (v) =>
    Math.round(Math.min(1, Math.max(0, (v - 0.5) / 0.35)) * 100)
      .toString()
      .padStart(3, "0")
  );
  ...
  React.useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const irisVisible = headOk ? v > 0.52 && v < 0.85 : v < 0.5;
...
/** Pupil radius over scroll: rest 0.26 → focus pin 0.12 → full dilation 1. */
function pupilAt(v: number): number {
  if (v < 0.52) {
    const t = Math.min(1, Math.max(0, (v - 0.36) / 0.16));
    return 0.26 - 0.14 * easeInOut(t);
  }
  if (v < 0.68) return 0.12;
  const t = Math.min(1, (v - 0.68) / 0.27);
